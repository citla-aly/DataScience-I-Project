...
format:
  html:
    ...
    self-contained: true #to preview this is false
    embed-resources: true #to preview this is false
    code-overflow: wrap
    code-tools: true
    toc: true
    toc-location: left
...
jupyter: python3
abstract: |
    This study investigates persistent cross-country variation in solvency conditions, financial structures, and asset allocations among European Union insurers under the harmonizing Solvency II regime. Using quarterly EIOPA data (2017-2024) covering Own Funds (SCR/MCR), Balance Sheet, and Operational Performance, the study employs exploratory data analysis and correlation to assess key dynamics. Results showed a segmented EU market, with Germany, France, and Austria exhibiting the highest average solvency indicators, partially explained by larger markets and robust regulation. High solvency is associated with lower loss and combined ratios, but profitability is only moderately correlated with solvency and remains in individual countries' risk management and assets allocation. While balance sheet flows show high proportionality (consistent with asset-liability management), these relationships weaken significantly when distinguishing Life and Non-Life sectors. Scatter-plot analysis confirms clustering driven by market size, showing that larger sectors hold greater absolute investments in bonds and equity. Overall, solvency strength, profitability, and investment patterns are jointly shaped by firm strategy and broader macroeconomic and demographic forces, confirming that national contexts remain the dominant source of variation despite common regulatory standards. 
---

# Introduction

Insurance markets contribute significantly to economic stability by transforming uncertain risks into predictable financial commitments. [@Arrow1965] describes, insurance represents the exchange of money today for money payable contingent upon the occurrence of certain events.

This essential function relies on the sector’s ability to maintain adequate capital buffers and withstand adverse conditions. According to the European Insurance and Occupational Pensions Authority (**EIOPA**), the **Solvency II** prudential regime, wich is in force since January 2016, establishes the requirements that insurance and reinsurance undertakings must meet to ensure adequate protection of policyholders and beneficiaries [@EIOPA].

Despite the existence of this harmonized regulatory framework, significant differences across countries persist within the European Union (EU). These divergences arise from economic, demographic, and structural characteristics, such as population size, gross domestic product (GDP) levels, mortality patterns, as well as more subjective elements including culture and risk appetite.

As a result, EU countries can exhibit varying solvency ratios [@EIOPAFinancialReport], financial structures, and capital management strategies shaped by national economic conditions, supervisory practices, and the composition of insurers’ balance sheets.
...
The motivation for this project was born from the need as future actuaries to understand the complex mechanism that exists in Solvency II indicators across EU countries. Therefore, this study conducts a comparative analysis of Solvency II indicators provided by EIOPA databases across EU countries.
...
- **Exploratory Data Analysis (EDA):** Perform a descriptive analysis of insurers’ financial structures for the year 2024, detailed by quarterly performance and univariate analysis of key variables. 

    - Key variables: Assets, Liabilities, Solvency Capital Requirement (SCR) ratio, Minimum Capital Requirement (MCR) ratio, etc.
...
Multiple lines of research have examined how insurers’ financial structures, investment decisions, and broader economic conditions influence solvency outcomes under the Solvency II framework. A broader macroeconomic perspective is offered by studies examining the insurance growth mechanism. In The *Relationship Between Insurance and Economic Growth: Evidence from the European Union Countries*, the authors analyse annual data from 2004 to 2015 to assess how insurance development interacts with economic growth [@VrdoljakRagu2020].  Their results show substantial cross-country heterogeneity: insurance sector development is generally higher in economically advanced countries, and the relationship between insurance penetration and economic growth varies significantly across member states. 

Expanding on this line of inquiry, Siopi and Poufinas (2023) adopt a broader perspective in *Impact of Internal and External Factors on Profitability and Financial Strength of Insurance Groups*. Studying 23 EU insurance groups from 2007 to 2021, they evaluate both profitability and financial strength (equity-to-assets) in relation to firm level characteristics and macroeconomic factors including GDP growth, inflation, long term interest rates, and market structure. Their key conclusion is that, once these determinants are taken into account, the introduction of Solvency II does not exert a statistically significant effect on either profitability or financial strength [@Siopi2023]. Instead, managerial efficiency, underwriting performance, portfolio composition, and broader economic conditions emerge as the dominant drivers. This result underscores that regulatory frameworks alone do not determine insurer performance; internal strategic decisions and macroeconomic environments often play a more decisive role.

Additional notable contributions include the study *Enterprise Risk Management and Solvency: The Case of the Listed EU Insurers* investigates the relationship between Enterprise Risk Management (ERM) adoption and solvency [@EnterpriseRisk].As well as *Solvency II assumptions for increasing the international competitiveness of the EU insurance industry* [@Peleckiene2014]. 

Collectively, these studies emphasise that solvency, profitability, and financial resilience within EU insurance markets are shaped by a combination of investment structure, internal management decisions, risk-management practices, and broader macroeconomic conditions. 
...
- **How do solvency patterns vary across the European Union countries and does solvency impact insurers’ profitability?**
...
- **[European Insurance and Occupational Pensions Authority (EIOPA)](https://www.eiopa.europa.eu/index_en)**. 
  ...
  Reports gross and net premiums, claims incurred, changes in technical provisions, and operating expenses for life and non-life insurance. It provides insight into insurers’ underwriting activity and operational performance.
...
- **[World Bank](https://data.worldbank.org/indicator/NY.GDP.MKTP.CD)** 
    ...
    - Gross Domestic Price - Per Capita
...
data = payload[1]  # position 0 = metadata, 1 = data
...
- Using `.info()` and .`describe()` to inspect variable types, reporting frequency, and the extent of missing values. [(See here.)](#missing-values-and-outliers)
...
- One single column identifies the variable (“Item Code”), with repeated values and one column contains the corresponding numerical value (“Value”), creating a long vertical data frame.

<img src="images/1.jpg" style="float:right; width:45%; margin-left:15px;">
...
- A suffix indicating the originating dataset was added (e.g., `R0500_B, R0110_P, R0200_O`) to retain traceability.
```{python}
...
#compute mean for ratios 
SCR_mean = (
    Own_Funds[Own_Funds['Item code'] == 'R0620_O']
    .groupby(['Reporting country', 'Date'])['Value']
    .mean()
    .rename('R0620_O')              # mismo nombre que la columna del pivot
    .reset_index()
)

#Sustitute the column of mean for ratios
Own_Funds_wide = (
    Own_Funds_sum
    .drop(columns=['R0620_O']) #we drop the column of sum
    .merge(SCR_mean, on=['Reporting country', 'Date'], how='left')
)

#Exclude European Union Agregate (EEA)
...
- Balance Sheet: Many missing values are inherent to the template (e.g., items not applicable to certain markets). These missing entries were not imputed unless analytically required.
...
- **1079 country-quarter observations** (rows)

- **159 wide-format financial indicators** (columns)
...
To determine solvency patterns across European Union countries, 4 key solvency factors ([Appendix 5.1](#appendix)).  are relevant under the Solvency II regulatory framework. 
...
- **Technical Provisions**: Specific category of liabilities that insurers must hold to cover expected future policyholder obligations. They are calculated based on the best estimate of future cash flows, adjusted for the time value of money and a risk margin [@EIOPA].

- **Solvency Capital Requirement Ratio (SCR Ratio)**: Measure that indicates whether an insurer holds sufficient eligible own funds to meet its Solvency Capital Requirement (SCR), which represents the level of capital that insurers must hold to ensure they can absorb unexpected losses with a 99.5% confidence level over a one-year horizon [@SCRDef]. 

- **Minimum Capital Requirement Ratio (MCR Ratio)**: Measures whether an insurer’s eligible own funds fall below the Minimum Capital Requirement, which represents the minimum level of capital an insurer must hold to avoid supervisory intervention [@MCRDef].

```{python} 
#Asign proper names for better understanding.
new_cols = pd.DataFrame({
    "Own_Funds": df["R0540_O"]* 1000,
    "SCR_ratio": df["R0620_O"],
    "MCR_ratio": df["R0640_O"],
    "Total_TP": (df[["R0510_BS", "R0600_BS", "R0690_BS"]].sum(axis=1)* 1000),
...
The SCR ratio and MCR ratio capture the same underlying solvency dimension ([Appendix 5.2](#appendix)). Therefore, the SCR ratio alone can serve as a sufficiently informative measure for the subsequent comparative analyses. 

Solvency levels are predominantly driven by external factors such as insurers’ risk profiles and additional supervisory practices [@PoufinasSiopi2024]. These differences may be partly influenced by geographical or country-specific factors; therefore, the next logical step is to examine a map plot to compare how these indicators vary across countries.
...
N = 5  # how many to show, the 5 more higer and 5 more lower just of the 2024 Q4
top_countries = df[df["year_quarter"] == "2024 Q4"].nlargest(N, "SCR_ratio")
bottom_countries = df[df["year_quarter"] == "2024 Q4"].nsmallest(N, "SCR_ratio")

df_lastquarter = df[df["year_quarter"] == "2024 Q4"].copy() #just last quarter
...
fig = px.choropleth(
    ...
    title="SCR Ratio by Country (2024 Q4)"
)

fig.update_geos(
    visible=False,
    lonaxis=dict(range=[-10, 30]), #region selection   
...
## Own Funds 
``` {python} 
top_OF= df[df["year_quarter"] == "2024 Q4"].nlargest(N, "Own_Funds")
bottom_OF= df[df["year_quarter"] == "2024 Q4"].nsmallest(N, "Own_Funds")
...
fig = px.choropleth(
    ...
    title="Own Funds Volume by Country (2024 Q4)"
)
...
``` {python} 
top_TP= df[df["year_quarter"] == "2024 Q4"].nlargest(N, "Total_TP")
bottom_TP= df[df["year_quarter"] == "2024 Q4"].nsmallest(N, "Total_TP")
fig = px.choropleth(
    map_df,
    geojson=map_df.geometry.__geo_interface__,
    locations=map_df.index,
    ...
    title="Total Technical Provisions by Country (2024 Q4)"
)
...
**Germany and Austria exhibit the highest average SCR ratios**, reflecting stronger solvency positions. Despite differences in market scale, both operate under robust supervisory frameworks alongside the Solvency II regime (institutions like **BaFin in Germany** and the **FMA in Austria**) which explains the highest solvency positions . Overall, EU countries display healthy SCR ratios.  

The Own Funds and Technical Provisions segmentation is more pronounced with a clear concentration in Central European countries. **France and Germany dominate with the largest volumes**. This is consistent with their status as two of the largest insurance markets in Europe and their high levels of insurance penetration [@FundacionMAPFRE2025]. 

Countries are classified into a high-solvency and a low-solvency group to evaluate whether the observed gap persists over time or merely reflects conditions in the most recent period . 
...
# Remove 2016
df_10SR = df_10SR[~df_10SR["year_quarter"].str.startswith("2016")]

# Remove 2025
df_10SR = df_10SR[~df_10SR["year_quarter"].isin(["2025 Q1", "2025 Q2"])]

# Without removed quarterly
order = sorted(df_10SR["year_quarter"].unique())
df_10SR["year_quarter"] = pd.Categorical(
    df_10SR["year_quarter"],
    categories=order,
    ordered=True
)
...
The graph reveals a consistent and stable gap over time: **high-solvency** countries maintain ratios between approximately **2.4 and 3.0**,  **low-solvency countries** remain in the **1.7-1.9** range with limited upward movement. However, a closer look suggests a recurring pattern of periodic fluctuation within the lines of both groups, indicating a potential **quarterly pattern or seasonality** that requires further statistical analysis. 

An interesting pattern emerges during the COVID-19 period: **high-solvency countries experienced a more notable temporary decline in their SCR ratios than low-solvency countries**. Although this may seem counterintuitive, it aligns with structural differences in market size, portfolio composition, and exposure to shocks, given that the main solvency shock during the pandemic came from investment-side impacts rather than claims [@Pulawska2021]. These investment-side dynamics will be examined in the subsequent research questions.

 <!-- Second part research question 1-->

### Does solvency impact insurers’ profitability?

To explore the connection between solvency levels and higher financial performance, key profitability indicators in the non-life insurance sector are defined as follows: 

- **Combined Ratio (CR)**: It measures the overall underwriting performance of non-life insurers.

- **Loss Ratio (LR)**: It measures the proportion of premiums that go toward paying claims.

- **Expense Ratio (ER)**: It measures how much of the premium income is consumed by operating expenses, such as commissions, underwriting costs,and administration.

Further information of formulas’ composition in [Appendix 6.3](#appendix). 

Although both the ER and the LR drive underwriting profitability, the variability in the Combined Ratio is primarily driven by the Loss Ratio, which may therefore be affected differently by solvency factors ([Appendix 6.4](#appendix)). Note that a profitability factor smaller implies a better underwriting profit.
...
sns.heatmap(
    ...
    cmap="coolwarm",
    fmt=".2f",
    annot_kws={"size": 10}, #size numbers
    cbar_kws={"shrink": 1} # size bar
)
...
The heatmap confirms **a strong negative correlation between the Loss Ratio and the Expense Ratio** (-0.87). Companies using low-cost distribution channels tend to rely on aggressive pricing and attract more sensitive risks, ultimately leading to higher incurred losses. Conversely, insurers with higher operating expenses must keep loss ratios lower in order to sustain profitability. 

First, **SCR ratio is negatively correlated with both the Combined Ratio and the Loss Ratio**. Insurers with higher SCR ratios tend to exhibit stronger underwriting performance, such as better risk selection and more disciplined pricing, resulting in lower loss and combined ratios.  Second, the **SCR ratio shows a small positive correlation with the Expense Ratio**. High-solvency insurers often operate multiple business lines, manage sophisticated investment portfolios, etc. Such operational complexity increases administrative costs leading to higher expense ratios.  However, **the magnitude of these correlations suggests that the impact of SCR ratio on profitability is not particularly strong**. Finally, **Own Funds and Technical Provisions are positively correlated with the Loss and Combined Ratios**. Large insurers handle greater portfolios and therefore experience a greater volume of claims, which can increase loss ratios and, consequently, combined ratios when premiums do not scale proportionately

Overall, these patterns confirm that solvency factors influence profitability ratios in different ways and that these correlations arise from multiple external factors. On the one hand, the SCR indicates that solvency positively affects profitability in terms of underwriting performance and the loss ratio, but negatively affects the expense ratio. On the other hand, size-related solvency factors (Own Funds and Technical Provisions) have a negative impact on insurers’ profitability. **These differences in the solvency-profitability relationship suggest that there is no consistent pattern regarding whether solvency factors exert a positive or negative influence on profitability**. 

In conclusion, the results of this first part support the hypothesis of Siopi and Poufinas, namely that Solvency II factors have no significant impact on profitability; instead, profitability is primarily shaped by external drivers such as asset allocation and effective risk management [@Siopi2023]. 

## Research Question 2
### How do levels of insurers’ assets and liabilities correlate with country-level premiums written and claims incurred across EU insurance markets? 

Understanding the relationship between **balance sheet components** (Assets & Liabilities) and **operational cash flows** (Premiums & Claims) is essential to analyse the financial stability of insurance markets. 


<img src="images/2.jpg" style="float:right; width:70%; margin-left:15px;">

Premiums represent inflows from policyholders, higher volumes of written premiums increase future claim obligations, thereby raising technical liabilities. At the same time, premiums generate immediately investable cash, which expands the asset base. Claims, conversely, are outflows that reduce available resources. Persistent or unusually large claims can consume assets investments or force insurers to adjust technical provisions, affecting both solvency and profitability.

Assets therefore capture accumulated premiums together with investment returns, while liabilities represent the expected value of future claims. The growth balance between these two elements directly influences solvency buffers and the insurer’s capacity to meet obligations without adjusting premiums. These relationships are naturally shaped by market conditions such as interest rates, investment performance, inflation, or exceptional loss events [@ZweifelEisen2012].

**EU Correlation Structure**

```{python}
# Separating Life and non life columns
...
## Overall Data
```{python}
# Configuration for only upper triangle of the matrix
...
plt.title(
    'Overall Data \n Life + Non Life + Other Undertakings',
    fontsize=12,
    fontweight='bold',
    pad=20
)
...
The correlation matrix reveals a high degree of connection across the EU insurance market as a whole. Three main patterns emerge:
 
1. **Assets and liabilities exhibit an almost perfect positive correlation of 100%.** This reflects effective Asset Liability Management practices across the region, strongly reinforced by Solvency II capital requirements and valuation standards.

2. **Premiums and claims show a correlation of 98%.** This confirms that underwriting activity and the resulting payouts move proportionally across countries, consistent with expected patterns of risk transfer in mature insurance markets.
...
- **Life insurance** displays **correlations** around **70%-72%**. According to Christophersen & Jakubik, life operations follow long term accumulation dynamics where **asset and liability** movements depend on **macroeconomic factors**, interest rate environments, and demographic trends [@ChristophersenJakubik2014]. 
...
The gap between the overall correlation and the line specific correlations suggests that the category labelled as "Other Undertakings" likely exhibits extremely high internal correlation, thereby lifting the general average.
...
df_corr = df_corr.dropna(subset=numeric_cols) 
df_corr = df_corr[(df_corr[numeric_cols] != 0).all(axis=1)] 
...
# Color palette
palette = px.colors.sequential.Greens[4::2] # ← change this to any 
color_map = {country: palette[i % len(palette)] for i, country in enumerate(countries)}

# Scatter plot lower triangle
for i in range(n):
    for j in range(i+1):
        fig.add_trace(
            go.Scatter(
...
palette = px.colors.sequential.Greys[2::2] 
color_map = {country: palette[i % len(palette)] for i, country in enumerate(countries)}
...
To complement the correlation coefficients, a scatter matrix was constructed to visualise the true dispersion of the data. Correlation alone does not communicate the **spread, slope, or clustering patterns** across countries.

The scatter matrix adds 2 key observations:

1. **Dominance of balance sheet variables and scale effects**. Assets and liabilities can reach values close to 3B euros, while premiums and claims generally remain below 300M. This clearly illustrates the distinction between stock variables and annual flow variables.

2. **Country clustering**. Colour coded points reveal distinct country clusters. France, Germany, and Italy appear consistently in the upper right region of each graph, reflecting their high premium income and investment capacity, driven by demographic scale and GDP levels. Smaller markets cluster near the origin, highlighting the concentration of financial activity across EU countries.
...
Finally, to gain a deeper understanding of country specific contexts (not just the large differences in magnitude across EU countries), we consider **insurance penetration** (premiums/GDP) against **economic development** (log GDP per capita).
...
# Add Slovakia manually GDP PER CAPITA
...
# Only annotate selected countries
...
# Fix label 
custom_handles = [
    Line2D([0], [0], marker='o', color='royalblue', markersize=10,
           linewidth=0, label='All Countries'),
    Line2D([0], [0], color='grey', linestyle='--', linewidth=1.5,
           label='Logistic Fit')
]

plt.legend(handles=custom_handles, loc='upper center')
plt.tight_layout()
```

The visual analysis confirms a **non-linear relationship**. This pattern aligns with theory, showing that insurance **growth accelerates** after a certain **wealth threshold** [@ZweifelEisen2012]. While most countries follow this trend, outliers like Luxembourg and Malta demonstrate that exogenous factors significantly influence penetration beyond mere economic development.

Overall, the data confirms that **operational cash flows strongly influence the insurance balance sheet**. Higher premiums drive corresponding increases in both liabilities (expected claims) and assets (reinvested inflows). Consequently, elevated claims temporarily reduce surplus, necessitating adjustments to technical provisions.
...
# France, Italy and Germany are the main volume premium countries in EU
...
# Recreate clusters for the cluster table
country_prem['Cluster'] = pd.qcut(
    country_prem['Total premiums written'],
    q=3,
    labels=['Low premiums', 'Medium premiums', 'High premiums']
...
# Add the 3 main countries line for the table
main_totals = (
    data_main
...
# Complete table with the 4 groups
...
# Dropdown to select the year of observation
buttons = []
...
The analysis conducted in this study shows that solvency indicators such as Own Funds, Technical Provisions, and capital requirement ratios (SCR and MCR) show strong and persistent segmentation. Markets like Germany and Austria maintain consistently higher technical provisions than less regulated countries. Solvency levels are linked to profitability but only moderately: higher solvency aligns with better underwriting performance like lower loss and combined ratios, yet profitability still depends heavily on underwriting strategy, operational structure, and business line rather than capital strength alone. 

Premiums and claims are closely related, indicating a mature and stable market. However, these relationships become weaker when Life and Non-Life Business Lines are examined separately, mainly due to the long-term horizon in Life Insurance and the short-term horizon and higher volatility in Non-Life Insurance. Resulting in different risk profiles and different risk underwriting strategies. The scatter plot reveals clear clusters, with France, Germany and Italy standing out due to their large asset bases, liabilities and premium volumes, which match the scale of their economies and populations. Insurance penetration offers a more meaningful comparison because, although these countries lead in absolute terms, their levels correspond to what would be expected given their economic activity. Luxembourg and Malta show unusually high insurance penetration relative to GDP per capita, suggesting a strong insurance culture. Ireland, Norway and Iceland appear under the line, indicating potential for market growth.

Patterns in asset allocation show that countries with larger insurance sectors and higher premium volumes hold higher absolute investments in bonds, and to a lesser extent, equities and property, with bonds remaining the dominant asset class in Europe due to regulatory capital requirements and the preference for stable, long-duration instruments. As premiums increase, insurers invest more in bonds, collective investments, and property, while derivative use remains largely unrelated to scale. 

Nevertheless, these findings come with **limitations**. This analysis relies on aggregated, country-level indicators, preventing us from capturing firm-level differences that may drive the observed patterns. Furthermore, all relationships identified are correlational, so we cannot claim causality at this stage. The 2017 to 2024 period contains substantial macroeconomic volatility, particularly due to the impact of Covid-19, which may have affected solvency indicators and investment strategies across insurers. Moreover, several important dimensions remain outside the scope of this study, including liquidity risk, market risk, and country-specific factors such as geographic exposure.

To advance this research, **future work** should incorporate more sophisticated econometric data, in order to distinguish correlation from causation. Given the segmentation observed between Life and Non-Life insurance, future work could model these business lines separately using long-horizon and short-horizon risk metrics to better characterize their distinct dynamics. Finally, analyzing the effects of macroeconomic shocks (Covid-19 period) through stress testing or time-series approaches could clarify how each country responds to periods of heightened volatility, offering insights for policymakers and supervisors.


# Appendix 
## Components of Solvency Factors Formulas

$$ 
\text{SCR Ratio} = \frac{\text{Eligible Own Funds}}{\text{Solvency Capital Requirement (SCR)}}
$$ {#eq-SCR-ratio}
...
$$
\text{Technical Provisions} = \text{Best Estimate (BE)} + \text{Risk Margin (RM)}
$${#eq-technical-provisions}

## Pairwise relationships across variables and distributions

```{python} 
sns.set_theme()
sns.pairplot(df[cols], diag_kind="hist", corner=True, height=2)
plt.show()
```

- **SCR ratio and MCR ratio exhibit an almost linear relationship**. 

- **Own Funds and Technical Provisions display clear clustering patterns**. These clusters reflect substantial differences in company size across European markets. 

Overall, the scatter-plot matrix points to a structurally segmented European insurance market, where solvency levels and firm size vary across countries. 

## Components of Profitability Factors Formulas

$$
\text{CR} = 
\frac{\text{Claims incurred} + \text{Expenses}}{\text{Premiums earned}}
$${#eq-technical-provisions}


$$
\text{LR} = 
\frac{\text{Claims incurred}}{\text{Premiums earned}}
$${#eq-technical-provisions}

$$
\text{ER} = 
\frac{\text{Expenses}}{\text{Premiums earned}}
$${#eq-technical-provisions}

## Boxplot of Profitability Factors
...
-  **Variability observed in the Combined Ratio appears to be primarily driven by the Loss Ratio**, as their distributions show more similar patterns of dispersion. The Expense Ratio, by comparison, exhibits lower variability across countries.

## Profitability Factors vs GDP per Capita 
