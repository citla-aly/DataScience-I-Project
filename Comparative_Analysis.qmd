...
!["Source: MAPFRE Economics (based on data from the annual reports of the groups specified)"](images/data4.png){fig-align="center" width="80%"}
...
- **Loss Ratio**: It measures the proportion of premiums that go toward paying claims.
...
- **Expense Ratio**: It measures how much of the premium income is consumed by operating expenses, such as commissions, underwriting costs, and administration.
...
The boxplots show that countries such as Malta and Lithuania exhibit significantly higher volatility in their Combined Ratio. This is consistent with smaller and less diversified markets, where isolated events can materially impact profitability results. In contrast, larger markets such as Germany, Norway, and the Netherlands display more stable distributions, reflecting a greater ability to absorb risk without loss profitability. 
...
To further explore these relationships and to understand how solvency indicators interact with profitability metrics a correlation matrix will be computed and visualized through a heatmap.
...
The heatmap confirms a strong negative correlation between the Loss Ratio and the Expense Ratio (-0.87). Companies using low-cost distribution channels tend to rely on aggressive pricing, which reduces margins and attracts more price-sensitive risks, ultimately leading to higher incurred losses. Conversely, insurers with higher operating expenses must keep loss ratios lower in order to sustain profitability.

The heatmap also reveals several additional relationships between solvency and profitability factors.

First, the **SCR ratio is negatively correlated with both the Combined Ratio and the Loss Ratio**. Insurers with higher SCR ratios tend to exhibit stronger underwriting performance, such as better risk selection and more disciplined pricing, resulting in lower loss and combined ratios. Second, the **SCR ratio shows a small positive correlation with the Expense Ratio**. Well-capitalized insurers often operate multiple business lines, manage sophisticated investment portfolios, etc. Such operational complexity increases commissions, administrative costs, and system expenses, leading to higher expense ratios. However, the magnitude of these correlations suggests that the impact of SCR ratio on profitability is not particularly strong.

Finally,  **Own Funds and Technical Provisions are positively correlated with the Loss and Combined Ratios**. Large insurers handle greater portfolios and therefore experience a greater volume of claims in absolute terms, which can increase loss ratios and, consequently, combined ratios when premiums do not scale proportionately. This indicates that size-related factors have a negative impact on insurers profitability.

Overall, these patterns indicate that solvency factors influence profitability ratios in different ways, and that these correlations arise from multifactorial mechanisms. Differences in underwriting strategy, operational structure, business models, and portfolio composition all contribute to the way solvency and performance indicators relate to one another. 
...
In conclusion, the results on this first part support the hypothesis of **Siopi and Poufinas** that Solvency II has no statistically significant impact either on profitability or on financial strength . [@Siopi2023]
